...
to create-city-from-gis
  ...
  gis:apply-coverage greenery "ID" parkid    ;; "DISTNAME1" contains the name of the park, but not all of them have it. Use the ID for the momen
...
to go
  ...
  if ticks > 0 and (ticks mod 6 = 0 or ticks mod 7 = 0) [
    ...
    if ticks mod 7 = 0 [
      ...
      foreach parks [pk -> update-park-attendance-weekly pk]
  ...
  foreach parks [pk ->
    ...
    if count people-here > 1 [
      ...
      ask people-here [ifelse iAmDissonant other people-here [
        let old precision prob-going 4
        set prob-going prob-going - (a * prob-going)
        if debug [show (word "DEBUG: age " age ", class " class " is dissonant!! old prob: " old " new prob: " precision prob-going 4)]
        ]
        [if debug [show (word "DEBUG: age " age ", class " class " is NOT dissonant!! prob-going: " precision prob-going 4)]]
      ]
    ]
  ]
  ;vid:record-view
...
to update-feedbacks-week
  ...
  ; ifelse [here-this-week] of pk >= [here-last-week] of pk
  let old precision prob-going 4
  ifelse item 0 table:get visits-to-parks closest-park >= item 1 table:get visits-to-parks closest-park
  [
    set prob-going prob-going + ((2 * a) * prob-going)
    if debug [show (word "DEBUG: Local park " closest-park " INCREASING attendance! - old prob: " old " new prob: " precision prob-going 4)]
  ]
  [
    set prob-going prob-going - ((2 * a) * prob-going)
    if debug [show (word "DEBUG: Local park " closest-park " DECREASING attendance! - old prob: " old " new prob: " precision prob-going 4)]
  ]
end
...
to print-debug-info
  print "Setup ended"
  print (word "City: " count patches with [not is-number? name] "; Agents: " count turtles)
  foreach table:keys class-t [cl -> print (word "Class " cl ": " count turtles with [class = cl])]
end
