...
turtles-own [
  ...
  has-car has-child has-dog work
]
...
to set-diversity-thresholds
  ifelse class > 2
  [set tol tolerance * 2]   ; the poor don't mind so much...
  [set tol tolerance]
...
to-report iAmDissonant [otherpeople pkq]
  ...
  ; If the park is bad everyone will hate it
  if have-quality and pkq = 0 [if random 100 < 80 [report true]]
...
to getLocalInfluence
  ...
  ask turtles [
    ...
    [set prob-going prob-going + (prob-going * a * b)]
    [if quantovanno < (visits * 0.75) [set prob-going prob-going - (prob-going * a * b)]]
  ]
end
...
to prepare-data-save
  ...
  let base (word "results/individual_runs/spans-" file-string "-" city "-p" initial-prob "-a" a "-b" b "-t" tolerance "-h" heteroph-tol walkab "-s" scale)
...
to save-final-stats
  ...
  let header-1 "city,age,class,pull,random,segregated,heterophily,equalinit,initial-prob,walkability,a,b,tolerance,heteroph-tol,medAB,medC1,medC2,medDE,meanAB,meanC1,meanC2,meanDE,"
  ...
  let conditions (word city "," push-age "," push-cl "," pull "," random-allocation ","
    complete-segregation "," have-heterophily "," same-init-prob "," initial-prob "," have-walkability ","
...
@#$#@#$#@
<experiments>
  <experiment name="allcities-heter" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
