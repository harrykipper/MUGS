...
globals [
  ...
  class-t dogs-t ind-file file-string area-file picture
...
patches-own [simd name parkid parksize parkquality green? smallpark? centre attractivity walkability]
...
to make-dogs-table
  ...
  (list
    (list "glasgow" (list 0.03 0.02 0.03 0.02))
    (list "edinburgh" (list 0.04 0.04 0.05 0.04))
    (list "dundee" (list 0.04 0.04 0.03 0.03))   ; 0 0.2 0.8 0.8
    (list "aberdeen" (list 0.05 0.03 0.05 0.06))
  )
end
          ...
          foreach agez [ag ->
            ...
            if howmany > 0 and any? places [
              crt howmany [
                ...
                set class cl
                set has-dog false
                if random-float 1 <= item (class - 1) table:get dogs-t city [set has-dog true]
                set Heter false
                if class > 2 [if random-float 1 <= prop-heter-c2de [set Heter True]]
...
ask patches gis:intersecting greenery [
    if is-string? parkid [
      set green? true
      set parkid substring parkid 0 13  ;; The park identifier is unreasonably long
    ]
  ]

  let parkz remove-duplicates [parkid] of patches with [green? = true]
  foreach parkz [  ;; Each park patch knows how big the whole park is. Useful later, maybe inefficient.
    pk ->
    let pks patches with [green? and parkid = pk]
    let sz count pks
    ifelse sz > 3      ;; WARNING: Parks of 3 patches and below are disregarded. May or may not have an effect...
    [ask pks [
      set parksize sz
      set smallpark? false
      ]
    ...
    [ask pks [
      ...
      set parkid 0
      set parksize 0
      set parkz remove pk parkz
      ]
    ]
  ]

  set parks patches with [green? = true] ; we make a patchset of all parks. MUCH more efficient than selecting every time...

  ; Questo non serve a un cazzo, ma me lo conservo perche' e' bello
  ; set parks sort-by [[pk1 pk2] ->
  ; [parksize] of one-of parkptchs with [parkid = pk1] > [parksize] of one-of parkptchs with [parkid = pk1]] parks

  set parkT table:from-list map [pk -> (list pk [parksize] of one-of parks with [parkid = pk])] parkz

  let medsize median table:values parkT

  foreach table:keys parkT [pk ->
    let these parks with [parkid = pk]
...
to identify-neighbours
  ;; People of my age in 500mt radius are "neighbours".
  let x (ifelse-value
  age >= 70 [20]
  age >= 50 [15]
  [10])
  set myNeighbours turtles in-radius (500 / patch-dimension) with [abs age - [age] of myself <= x]
end

; ===========| Main loop |=========================

to go
  ; if impossible-run [stop]  ;; Uncomment this if running in BS with impossible combinations in the mix..
  ;vid:record-view
  ask turtles [
    move-to casa
  ...
  ask turtles with [has-dog = true][
    if random-float 1 < 0.9 [
      ifelse random-float 1 < 0.75 or length myLocalParks = 1
      [go-outdoors first myLocalParks]
      [go-outdoors one-of but-first myLocalParks]
    ]
  ]
  ask turtles with [has-dog = false] [
...
to evaluate-agents-experience
  ...
  foreach table:keys parkT [pk ->
    let people-here turtles-on parks with [parkid = pk]
      ...
      ask people-here [
        ...
        [
          ...
          ifelse length myLocalParks > 1
            ...
            [set multiplier 1] ;1.20]  ;; if we only have one park that we can go to and it disappoints us we are going to suffer more. Turn it off please
...
to-report iAmDissonant [otherpeople pkq]
  ...
  let fc factor
  if [parksize] of patch-here > (1000 ^ 2) / (patch-dimension ^ 2) [set fc fc * 2]
...
to getLocalInfluence
  ...
  ask turtles [
    ...
    [if quantovanno < (visits * 0.75) [set prob-going prob-going - (prob-going * a * influ)]]
  ]
end
...
to go-outdoors [where]
  move-to one-of parks with [parkid = where]
  set visits visits + 1
  set gone-today true
...
to-report impossible-run
  if (push-cl = false and pull = false and have-quality = false) or
     (random-allocation = true and complete-segregation = true)
  [report true]
  report false
end
...
to-report get-parkquality [dep]
  if dep > 3 [
    ifelse random-float 1 < 0.8 [report 4][report 2]
  ]
  ifelse dep > 1
  [ifelse random-float 1 < 0.8 [report 2][report 4]]
  [ifelse random-float 1 < 0.7 [report 0][report 2]]
end
...
to prepare-data-save
  ...
  let base (word "results/individual_runs/spans-" file-string "-" city "-p" initial-prob "-a" a "-b" b "-t" tolerance "-h" heteroph-tol "-pH" prop-heter-c2de walkab "-s" scale)
...
to save-final-stats
  ...
  let header-1 "city,age,class,pull,random,segregated,heterophily,equalinit,initial-prob,walkability,a,b,tolerance,heteroph-tol"
  let header-1A ",medAB,medC1,medC2,medDE,meanAB,meanC1,meanC2,meanDE,"
  let header-2 "varAB,varC1,varC2,varDE,med1,med2,med3,med4,med5,mean1,mean2,mean3,mean4,mean5,"
  let conditions (word city "," push-age "," push-cl "," pull "," random-allocation ","
    complete-segregation "," prop-heter-c2de "," same-init-prob "," initial-prob "," have-walkability ","
    a "," b "," tolerance ","  heteroph-tol ",")
  ...
  foreach table:keys parkT [pk ->
    file-type (word conditions "," pk "," table:get parkT pk "," getShannonPk pk)
    file-print ""
  ]
  file-close
...
* Introduce dogs. People with dogs will go almost every day (p=0.90), mostly to the local park (p=0.75), irrespective of... quality!
* Introduce park quality: 
-- Parks in city centres are always well maintained
-- small parks in deprived areas are more likely to be undermaintained
-- bigger parks are mostly well maintained


## 0.4.4

* Reintroduce age differences
* Measure park attendance by class
* Implement age-based local influence

## 0.3

* discount the fraction of the people we encounter by the size of the park
* use a different value of walkability quintiles and make the influence sharper
* take more values in the final output file
* so many more....
...
@#$#@#$#@
<experiments>
  <experiment name="master" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="quality" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
  ...
  <experiment name="b" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
  ...
  <experiment name="extremes" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
  ...
  <experiment name="edin-gla-rand" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
    ...
    <enumeratedValueSet variable="edinburgh-test">
      <value value="true"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
