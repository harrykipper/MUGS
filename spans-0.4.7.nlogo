...
patches-own [simd name parkid parksize parkquality green? attractivity walkability]
...
turtles-own [
  ...
  ; id has-car has-child has-dog work
  Heter
]
...
to setup
  ...
  set version "0.4.7r"
  if push-cl = false [set version (word version "-noClass")]
  set classes [1 2 3 4]
  ...
  ;; visits-to-parks is a table keeping track of what kind of people
  ;; visited each park.
  ;; park-name | ab | c1 | c2 | de
  foreach parks [pk -> table:put visits-to-parks pk [0 0 0 0]]
...
to create-city-from-gis
  ...
  foreach parks [  ;; Each park patch knows how big the whole park is. Useful later, maybe inefficient.
    ...
    let pkq get-parkquality one-of modes [simd] of pks
    let sz count pks
    ask pks [
      set parksize sz
      set parkquality pkq
...
to evaluate-agents-experience
  ...
  foreach parks [pk ->
    ...
    table:put visits-to-parks pk park-goers pk people-here
...
to-report iAmDissonant [otherpeople pkq]
  ...
  if have-quality [
    if pkq = 0 [if random 100 < 80 [report true]] ; If the park is bad (almost) everyone will hate it
  ]

  if push-age [
    if age >= 65 [if count others with [age <= 30] / count others >= 0.5 [report true]]
  ]
...
to-report park-goers [park people-here]
  let old-count table:get visits-to-parks park
...
to-report getShannonPk [park]
  let all table:get visits-to-parks park
  ifelse all != [0 0 0 0] [
    let div map
...
to save-final-stats
  ...
  foreach parks [pk ->
    file-type (word conditions "," pk "," table:get visits-to-parks pk "," getShannonPk pk)
    file-print ""
  ]
  file-close
