...
to print-debug-info
  ...
  foreach table:keys class-t [
    ...
    let allwalk remove-duplicates [walkability] of patches
    foreach allwalk [wk ->
...
to prepare-data-save
  ...
  set ind-file  (word "results/individual_runs/spans-" file-string "-" city "-prob" initial-prob "-a" a "-tol" tolerance "-w" walkab "-s" scale ".csv")
  set area-file (word "results/individual_runs/spans-" file-string "-" city "-prob" initial-prob "-a" a "-tol" tolerance "-w" walkab "-s" scale "-areas.csv")
  file-open ind-file
...
to save-final-stats
  ...
  let header-1 "city,age,class,pull,random,segregated,heterophily,equalinit,initial-prob,walkability,a,tolerance,medAB,medC1,medC2,medDE,meanAB,meanC1,meanC2,meanDE,"
  let header-2 "varAB,varC1,varC2,varDE,med1,med2,med3,med4,med5,mean1,mean2,mean3,mean4,mean5,"
  let conditions (word city "," push-age "," push-cl "," pull "," random-allocation "," complete-segregation "," have-heterophily "," same-init-prob "," initial-prob "," have-walkability "," a "," tolerance ",")
