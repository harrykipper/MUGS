...
globals [
  ...
  class-t ind-file file-string area-file
...
to setup
  ...
  ; certain combinations of cases don't make sense
  if (push-cl = false and pull = false) or (random-allocation = true and complete-segregation = true) [stop]
  ...
  if not same-init-prob [set class-t table:from-list [[1 [1 yellow]] [2 [0.8 blue]] [3 [0.6 black]] [4 [0.4 red]]]]
  ...
  [prepare-data-save]  ;; If we are in batch mode we produce an output file with everything in it.
  [colour-world]
  ;vid:start-recorder
end
      ...
      foreach sex [sx ->
        foreach classes [cl ->
          foreach agez [ag ->
            ...
            let places patches with [green? = false and is-string? name]
            if random-allocation = false [
              ifelse complete-segregation
              [set places places with [simd = getSimd cl]]
              [set places places with [name = item 0 zn and walkability > 0]]
            ]
...
to create-city-from-gis
  ...
  gis:apply-coverage walk "QUARTILE" walkability
...
to set-diversity-thresholds
  set tol random-normal ((min list (155 - age) 100 / 100) * tolerance) 0.10 ;; Tolerance decreases with age. After 55 people get grumpy
  if have-heterophily and class > 2 [
    let tl tolerance
    ;; If the poor are heterophiliac the minimum amount of rich people they will require to be happy won't
    ;; exceed 25%. It gets a bit silly otherwise.
    if tl > 0.25 [set tl 0.25]
    set tol random-normal tl 0.10
  ]
end
...
to set-preferred-parks
  ...
  ask turtles [
    ...
    if length myLocalParks = 0 [set prob-going prob-going * 0.5]
...
to evaluate-agents-experience
  ...
  ;; If the mix was acceptable the likelihood of going again to the same park will go up a bit (a/2)
...
to-report iAmDissonant [otherpeople]
  ...
  ;; Here we *REALLY* should discount the fraction of the people we encounter by the size of the park.
  ...
  if push-cl [
    ...
    [ ; the poor
      ...
      [if differing < tol [report true]] ;; if we have heterophily the poor fancy being around the rich
      [if differing > tol [report true]] ;; if not, the poor behave exactly like the rich and want to be around similar people
    ][; the rich
...
;  ;; This checks 'how-many-neighbours' random neighbours. If they have gone more than us, we will be encouraged.
;  ;; If they have gone much less than us, we will be discouraged.
;  ask turtles [
;    let ppl up-to-n-of 5 my-neighbours
;    if global-perception [set ppl n-of 7 other turtles]
;    let quantovanno median [visits] of ppl
;    ifelse quantovanno > (visits * 1.20)
;    [set prob-going prob-going + ((a * 0.5) * prob-going)]
;    [if quantovanno < (visits * 0.70) [set prob-going prob-going - ((a * 0.5) * prob-going)]
;    ]
;  ]
;end
...
to getLocalInfluence
  ...
  ask turtles [
    ;let multiplier 0.5
    ;if class > 2 [set multiplier multiplier * 2] ;; The poor are more sensitive to local area pressure.
    ;let ppl one-of my-neighbours
    ;if global-perception [set ppl one-of other turtles]
    let quantovanno [visits] of one-of myNeighbours
    ifelse quantovanno > (visits * 1.20)
    [set prob-going prob-going + (a * prob-going)]
    [if quantovanno < (visits * 0.80) [set prob-going prob-going - (a * prob-going)]]
  ]
end
...
to-report get-attractivity [pksize]
  ...
  report (ifelse-value
    pksize < (200 ^ 2) / (patch-dimension ^ 2) [200 / patch-dimension]
      ...
      pksize < (1500 ^ 2) / (patch-dimension ^ 2) [1000 / patch-dimension][1500 / patch-dimension]
  )
end
...
to prepare-data-save
  ...
  ;if global-perception = true [set global "_global"]
  ...
  set ind-file  (word "results/individual_runs/spans-" file-string "-" city "-prob" initial-prob "-a" a "-tol" tolerance "-s" scale "-run_" behaviorspace-run-number ".csv")
  set area-file (word "results/individual_runs/spans-" file-string "-" city "-prob" initial-prob "-a" a "-tol" tolerance "-s" scale "-run_" behaviorspace-run-number "-areas.csv")
  file-open ind-file
...
to save-final-stats
  ...
  let file-name (word "results/spans-" version "-scale_" scale "-all.csv")
  let zone-file (word "results/spans-" version "-scale_" scale "-zones.csv")
  let header-1 "city,age,class,pull,random,segregated,heterophily,equalinit,initial-prob,a,tolerance,medAB,medC1,medC2,medDE,meanAB,meanC1,meanC2,meanDE,"
  let header-2 "varAB,varC1,varC2,varDE,med1,med2,med3,med4,med5,mean1,mean2,mean3,mean4,mean5,"
  let conditions (word city "," push-age "," push-cl "," pull "," random-allocation "," complete-segregation "," have-heterophily "," same-init-prob "," initial-prob "," a "," tolerance ",")
  ...
  foreach quintiles [qtl ->
    ifelse count patches with [simd = qtl] > 0  ;; we have to do this beacause some cities don't have all the deprivation quintiles
    [file-type (word (median [visits] of turtles-on patches with [simd = qtl] / years) ",")]
    [file-type "NA,"]
  ]
  foreach quintiles [qtl ->
      ifelse count patches with [simd = qtl] > 0
  ...
  [
    file-open zone-file
    file-print (word "zone,diversity,ab,c1,c2,de," header-1)
  ]
  foreach zones [zn ->
...
@#$#@#$#@
<experiments>
  <experiment name="abe" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="dun" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
  ...
  <experiment name="edi" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
  ...
  <experiment name="gla" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
  ...
  <experiment name="allcities" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
  ...
  <experiment name="allcities_supashort" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
  ...
  <experiment name="abegla_short" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
  ...
  <experiment name="edidun_short" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
  ...
  <experiment name="bigcities_supashort" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 1 + (365 * years)</exitCondition>
    <enumeratedValueSet variable="tolerance">
