...
globals [
  ...
  class-t ind-file area-file
  classes quintiles prob-init
]

patches-own [simd name weather parkid parksize green? attractivity]
...
to setup
  ...
  gis:set-world-envelope-ds gis:envelope-of place

  set visits-t table:make
  set visits-to-parks table:make
  if not same-init-prob [set class-t table:from-list [[1 1] [2 1.2] [3 1.4] [4 1.7]]]  ; Class exponent for culture

  create-city-from-gis
  read-agents
  set-preferred-parks
  foreach zones [zn -> table:put visits-t zn [0 0]]
...
to create-city-from-gis
  ...
  gis:apply-coverage greenery "ID" parkid ;; "DISTNAME1" in the shp contains the name of the park, but not all of them have it. Use the "ID" for the momen
  ...
  ask patches with [green? and parksize < 3] [
    set green? false
    set parkid 0
    set parksize 0
  ]
...
to go
  ...
  ; We update certain things on Sundays.
...
to evaluate-agents-experience
  ...
  ;; Here all those who have gone to a park check whether they liked who else was there.
  ;; If an agent is dissonant, her likelihood to visit the park will decrease of factor 'a'
  ;; and will move the offending park to the bottom of the list of accessible parks,
  ;; so that next time hel'll go to a different place.
  ;; If the mix was acceptable the likelihood of going again will go up a bit (a / 2)
  ;; and the agent will keep going to the same park.
    ...
    if count people-here > 1 [
      ...
      ask people-here [
        ifelse iAmDissonant other people-here [
          ...
          set local-parks remove-item 0 lput pk local-parks]
        [set prob-going prob-going + ((a * 0.5) * prob-going)]   ;; With this we want to reinforce the pleasure of seeing others like us.
      ]
    ]
  ]
end
...
;; =================| 3 different ways of implementing "social influence" |================================

;to update-feedbacks-week

  ;; This checks the habits in someone's local area. If the number of people visiting parks
  ;; in a certain area is above the city average then all residents of the area will increase
  ;; their likelihood of going next week of factor 'a'.

;  let prop table:get visits-t [name] of casa
;  if prop > table:get visits-t "overall" [set prob-going prob-going + (a * prob-going)]
;end

;to update-feedbacks-week

  ;; This checks variations in the attendance to someone's local park.
  ;; If this week more people have gone to the park closest to the agent compared to last week,
  ;; she will increase her likelihood of going next week of factor 'a'

;  if item 0 table:get visits-to-parks closest-park >= item 1 table:get visits-to-parks closest-park
;  [set prob-going prob-going + (a * prob-going)]
;end

to update-feedbacks-week

  ;; This checks variations in the attendance to parks in someone's local area.
  ;; If this week more people have gone to the park compared to last week, all residents of the area
  ;; will increase their likelihood of going next week of factor 'a'.
  ;; Currently we consider at least 10% increases.

  let area [name] of casa
  if item 0 table:get visits-t area > (item 1 table:get visits-t area + (item 1 table:get visits-t area * 0.10))
  [set prob-going prob-going + (a * prob-going)]
end
...
to go-outdoors
  ...
  ;; see evaluate-agents-experience for the rest
  let wheretogo first local-parks
...
to print-debug-info
  ...
  foreach table:keys class-t [cl -> show (word "Class " cl ": " count turtles with [class = cl])]
end
...
to update-park-attendance-weekly [park]
  let lastweek item 0 table:get visits-to-parks park
  table:put visits-to-parks park list 0 lastweek
end

to update-zone-attendance-weekly [zone]
  let lastweek item 0 table:get visits-t zone
  table:put visits-t zone list 0 lastweek
end

to update-visits-zone
  ;; Keep track of proportion of park goers per zone. Weekly
  foreach zones [z ->
...
# Changelog
## 0.2.3
The choice of parks to visit is not completely random anymore. Now the accessible parks are in a list, the agent picks the first item and if the experience is not pleasant the park will be moved to the bottom of the list, so that next time she will go to a different place.
